...
self.onmessage = async (e) => {
  ...
  if (type === 'detect') {
    try {
      ...
      tryDetect(prep.baseMat, "");

      if (typeof setUploadProgress === "function") {
        setUploadProgress(90);
      }
      tryDetect(prep.flippedMat, "[rot180] ");
      ...
      candidates.sort((a, b) => {
        ...
        if (isPhoto && aMatchesHint && bMatchesHint && strongMa === 0 && strongMb === 0) {
          ...
          const aEndpointTicks = Boolean(da.orientationDebug && typeof da.orientationDebug.snapMode === 'string' && da.orientationDebug.snapMode.includes('+endpoint-ticks'));
          const bEndpointTicks = Boolean(db.orientationDebug && typeof db.orientationDebug.snapMode === 'string' && db.orientationDebug.snapMode.includes('+endpoint-ticks'));
          if (Math.max(a.mat.cols, a.mat.rows, b.mat.cols, b.mat.rows) <= 900 && aHorizontal && bHorizontal && da.detectedLengthMm === db.detectedLengthMm && aEndpointTicks !== bEndpointTicks) {
      ...
      const displayMat = (best.label && best.label.includes('[rot180]'))
        ? prep.baseMat
        : best.mat;
      ...
      if (
        isHorizontal &&
        (!sourceMeta || !sourceMeta.isPdf) &&
        best.label && best.label.includes('[rot180]') &&
        best.detection.method && best.detection.method.includes('+ocr')
      ) {
        const midY = (p0.y + p12.y) / 2;
        if (outputCanvas.height >= 2000 && midY < outputCanvas.height * 0.18) {
          const yShift = Math.max(20, Math.min(50, outputCanvas.height * 0.01));
          p0 = { ...p0, y: Math.max(0, Math.min(outputCanvas.height - 1, p0.y + yShift)) };
          p12 = { ...p12, y: Math.max(0, Math.min(outputCanvas.height - 1, p12.y + yShift)) };

          const cx = (p0.x + p12.x) / 2;
          const cy = (p0.y + p12.y) / 2;
          const expand = 1 + (yShift / Math.max(1, outputCanvas.height * 0.335));
          p0 = {
            x: Math.max(0, Math.min(outputCanvas.width - 1, cx + (p0.x - cx) * expand)),
            y: Math.max(0, Math.min(outputCanvas.height - 1, cy + (p0.y - cy) * expand)),
          };
          p12 = {
            x: Math.max(0, Math.min(outputCanvas.width - 1, cx + (p12.x - cx) * expand)),
            y: Math.max(0, Math.min(outputCanvas.height - 1, cy + (p12.y - cy) * expand)),
          };
          appliedTopPhotoShift = true;
        }
      }

      // Photo-specific fallback: some flipped top-ruler photos land on a parallel line
      // above the intended tick baseline when OCR-start exists but no OCR anchors are confirmed.
      if (
        isHorizontal &&
        best.label && best.label.includes('[rot180]') &&
        orientationDebug && orientationDebug.ocrStart &&
        !appliedTopPhotoShift &&
        (orientationDebug.ocrMatchedCount || 0) === 0
      ) {
        const midY = (p0.y + p12.y) / 2;
        if (midY < outputCanvas.height * 0.25) {
          const yShift = Math.max(20, Math.min(50, outputCanvas.height * 0.01));
          p0 = { ...p0, y: Math.max(0, Math.min(outputCanvas.height - 1, p0.y + yShift)) };
          p12 = { ...p12, y: Math.max(0, Math.min(outputCanvas.height - 1, p12.y + yShift)) };

          if (!(best.detection.method && best.detection.method.includes('+ocr'))) {
            // Approximate perspective widening when moving from upper to lower ruler edge.
            const cx = (p0.x + p12.x) / 2;
            const cy = (p0.y + p12.y) / 2;
            const expand = 1 + (yShift / Math.max(1, outputCanvas.height * 0.14));
            p0 = {
              x: Math.max(0, Math.min(outputCanvas.width - 1, cx + (p0.x - cx) * expand)),
              y: Math.max(0, Math.min(outputCanvas.height - 1, cy + (p0.y - cy) * expand)),
            };
