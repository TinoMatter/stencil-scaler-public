...
function detect(mat, sourceMeta, rulerLengthMm) {
  try {
    ...
    const ranking = (typeof module !== 'undefined' && module.exports)
      ? require('./candidate-ranking.js')
      : (typeof globalThis !== 'undefined' ? globalThis.candidateRanking : undefined);
    ...
    refinedCandidates.sort((a, b) => {
      ...
      if ((!sourceMeta || !sourceMeta.isPdf) && Math.max(mat.cols, mat.rows) <= 900 && aMatchesHint && bMatchesHint && aMatches === 0 && bMatches === 0) {
        const t = ranking.comparePhotoHorizontalTieBreaks(
          {
            dx: aDx, dy: aDy, score: a.score || 0,
            detectedLengthMm: a.detectedLengthMm,
            isBand: Boolean(a.isBand),
            isLineFallback: a.method.includes('Linien-Fallback'),
            isTick: a.method.includes('Tick-Cluster'),
            hasEndpointTicks: Boolean(a.orientationDebug && typeof a.orientationDebug.snapMode === 'string' && a.orientationDebug.snapMode.includes('+endpoint-ticks')),
            frameMaxDim: Math.max(mat.cols, mat.rows),
          },
          {
            dx: bDx, dy: bDy, score: b.score || 0,
            detectedLengthMm: b.detectedLengthMm,
            isBand: Boolean(b.isBand),
            isLineFallback: b.method.includes('Linien-Fallback'),
            isTick: b.method.includes('Tick-Cluster'),
            hasEndpointTicks: Boolean(b.orientationDebug && typeof b.orientationDebug.snapMode === 'string' && b.orientationDebug.snapMode.includes('+endpoint-ticks')),
            frameMaxDim: Math.max(mat.cols, mat.rows),
          }
        );
        if (t !== 0) return t;
